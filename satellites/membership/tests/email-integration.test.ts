...
describe("Membership Email Integration", () => {
  ...
  const mockCore: any = {
    ...
    container: {
      ...
      make(key: string) {
        if (key === 'i18n') return { t: (k: string) => k };
  ...
  beforeAll(async () => {
    ...
    mockCore.hooks.addAction('membership:send-verification', async (data: any) => {
       await signal.send(new (await import("../src/Application/Mail/WelcomeMail")).WelcomeMail(data.email, data.token));
    });
  });
  ...
  it("觸發忘記密碼應發送重設郵件", async () => {
    ...
    mockCore.hooks.addAction('membership:send-reset-password', async (data: any) => {
       await signal.send(new (await import("../src/Application/Mail/ForgotPasswordMail")).ForgotPasswordMail(data.email, data.token));
    });
  ...
  it("等級變更後應發送晉級通知郵件", async () => {
    mockCore.hooks.addAction('membership:level-changed', async (data: any) => {
       await signal.send(new (await import("../src/Application/Mail/MemberLevelChangedMail")).MemberLevelChangedMail(
         data.email, data.oldLevel, data.newLevel
       ));
    });
