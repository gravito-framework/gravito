...
export function createTracingMiddleware(tracer: TracingManager) {
    return async (c: GravitoContext, next: () => Promise<void>): Promise<Response | undefined> => {
        ...
        try {
            ...
            tracer.setAttribute(span, 'error.message', error instanceof Error ? error.message : 'Unknown error')
...
export { TracingManager, type Span, type SpanContext, type SpanEvent } from './TracingManager'
