...
describe('OrbitDB', () => {
  ...
  it('should register db and hooks', () => {
    ...
    expect(mockDoAction).toHaveBeenCalledWith('db:connected', expect.objectContaining({ db: mockDb }));
    expect(mockUse).toHaveBeenCalled();
  });

  it('should inject DBService into context', async () => {
    const mockDb = { name: 'mock-db', select: () => ({ from: () => ({ limit: () => Promise.resolve([]) }) }) };
  ...
  describe('DBService', () => {
    ...
    beforeEach(() => {
      ...
      orbitDB(core, { db: mockDb });
      const middleware = (mockUse as any).mock.calls[0][1];
      const mockContext: any = { set: mock((key: string, value: any) => { mockContext[key] = value; }) };
      middleware(mockContext, async () => {});
    ...
    it('should support findOneWith', async () => {
      ...
      const result = await dbService.findOneWith('users', { email: 'john@example.com' }, { profile: true });
    ...
    it('should support migrateTo', async () => {
      ...
      expect(result.appliedMigrations).toEqual(mockMigrations);
      expect(mockDb.migrate).toHaveBeenCalled();
      expect(mockDoAction).toHaveBeenCalledWith('db:migrate:start', expect.objectContaining({ targetMigration: '001_initial' }));
    });
    ...
    it('should support seed', async () => {
      ...
      expect(mockDoAction).toHaveBeenCalledWith('db:seed:start', expect.objectContaining({ seedName: 'test-seed' }));
  ...
  describe('Hooks', () => {
    ...
    it('should trigger db:query hook when query logging is enabled', async () => {
      ...
      expect(mockDoAction).toHaveBeenCalledWith('db:query', expect.objectContaining({ query: expect.any(String) }));
    });
  });
  ...
  describe('Model', () => {
    ...
    it('should find model by id', async () => {
      ...
      // Mock findById
      const mockFindById = mock(() => Promise.resolve({ id: 1, name: 'John', email: 'john@example.com' }));
    ...
    it('should find model by column', async () => {
      ...
      const mockFindOne = mock(() => Promise.resolve({ id: 1, name: 'John', email: 'john@example.com' }));
      ...
      expect(user).not.toBeNull();
      expect(user?.get('email')).toBe('john@example.com');
      expect(mockFindOne).toHaveBeenCalledWith({ _: { name: 'users' } }, { email: 'john@example.com' });
    });
    ...
    it('should create model', async () => {
      ...
      const mockCreate = mock(() => Promise.resolve({ id: 1, name: 'John', email: 'john@example.com' }));
      ...
      expect(mockCreate).toHaveBeenCalledWith({ _: { name: 'users' } }, { name: 'John', email: 'john@example.com' });
    });
    ...
    it('should save model (create)', async () => {
      ...
      const mockCreate = mock(() => Promise.resolve({ id: 1, name: 'John', email: 'john@example.com' }));
      (dbService as any).create = mockCreate;
      (dbService as any).findById = mock(() => Promise.resolve({ id: 1, name: 'John', email: 'john@example.com' }));
    ...
    it('should save model (update)', async () => {
      ...
      const mockUpdate = mock(() => Promise.resolve([{ id: 1, name: 'Updated', email: 'john@example.com' }]));
      const mockFindById = mock(() => Promise.resolve({ id: 1, name: 'Updated', email: 'john@example.com' }));
    ...
    it('should check if model exists', async () => {
      ...
      expect(mockExists).toHaveBeenCalledWith({ _: { name: 'users' } }, { email: 'john@example.com' });
    });
