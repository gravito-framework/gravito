...
export interface ModelStatic<T extends Model = Model> {
  ...
  all(options?: { limit?: number; orderBy?: unknown; orderDirection?: 'asc' | 'desc' }): Promise<ModelCollection<T>>;
  findAll(where?: WhereCondition, options?: { limit?: number; orderBy?: unknown; orderDirection?: 'asc' | 'desc' }): Promise<ModelCollection<T>>;
  ...
  paginate(options: { page: number; limit: number; orderBy?: unknown; orderDirection?: 'asc' | 'desc' }): Promise<{ data: T[]; pagination: { page: number; limit: number; total: number; totalPages: number; hasNext: boolean; hasPrev: boolean } }>;
}
...
export abstract class Model<TAttributes = Record<string, unknown>> {
  ...
  static setDBService(dbService: DBService): void {
    (this as unknown as typeof Model).dbService = dbService;
  }
  ...
  protected static getTable(): Table {
    ...
    if (!table) {
      throw new Error(`[Model] Table not set for ${this.name}. Please set static table property.`);
    }
    return table;
  }
  ...
  protected static getDBService(): DBService {
    ...
    if (!dbService) {
      throw new Error(`[Model] DBService not set for ${this.name}. Please call Model.setDBService() or ensure db:connected hook is triggered.`);
    }
    return dbService;
  }
  ...
  static async where<T extends Model>(
    ...
    return this.whereMany({ [column]: value });
  }
  ...
  static async whereMany<T extends Model>(
    ...
    return (this as any).fromData(data) as T;
  }
  ...
  static query<T extends Model>(this: ModelStatic<T>): QueryBuilder<T> {
    return new QueryBuilder<T>(this as ModelStatic<T>);
  }
  ...
  static async findAll<T extends Model>(
    ...
    // biome-ignore lint/suspicious/noExplicitAny: generic model creation
    const models = data.map((item: any) => (this as any).fromData(item));
  ...
  static async paginate<T extends Model>(
    ...
    return {
      // biome-ignore lint/suspicious/noExplicitAny: generic model creation
      data: result.data.map((item: any) => (this as ModelStatic<T>).fromData(item)),
  ...
  static async create<T extends Model>(
    ...
    const instance = (this as ModelStatic<T>).fromData(created);
    (instance as any).wasRecentlyCreated = true;
    
    // 觸發 created 事件
    const modelClassInstance = this as unknown as typeof Model;
    if (modelClassInstance.core) {
      await modelClassInstance.core.hooks.doAction('model:created', { model: instance });
      await modelClassInstance.core.hooks.doAction('model:saved', { model: instance, wasRecentlyCreated: true });
    }
  ...
  get<K extends keyof TAttributes>(key: K): TAttributes[K] | undefined {
    ...
    if (casts[String(key)] && value !== undefined && value !== null) {
      return (this.constructor as typeof Model).castAttribute(String(key), value, casts[String(key)]) as TAttributes[K];
    }
  ...
  async save(): Promise<this> {
    ...
    // 觸發 saving 事件
    if (modelClass.core) {
      await modelClass.core.hooks.doAction(
        isCreating ? 'model:creating' : 'model:updating',
        { model: this, attributes: this.attributes }
      );
    }
    ...
    // 觸發 saved 事件
    if (modelClass.core) {
      await modelClass.core.hooks.doAction('model:saved', { model: this, wasRecentlyCreated: (this as any).wasRecentlyCreated });
    }
...
export class QueryBuilder<T extends Model> {
  ...
  async first(): Promise<T | null> {
    ...
    // 應用全域作用域和軟刪除
    let finalWhere = this.buildWhere();
   ...
   * 分頁
   */
  async paginate(page: number, limit: number): Promise<{ data: ModelCollection<T>; pagination: { page: number; limit: number; total: number; totalPages: number; hasNext: boolean; hasPrev: boolean } }> {
    const modelClass = this.modelClass as unknown as typeof Model;
    const dbService = (modelClass as any).getDBService();
    const table = (modelClass as any).getTable();
    ...
    const models = new ModelCollection(result.data.map((item: any) => (this.modelClass as any).fromData(item)) as T[]);
...
export class ModelRegistry {
  ...
  static register(model: ModelStatic, table: Table, tableName: string): void {
    this.models.push({ model, table, tableName });
  }
  ...
  static initialize(dbService: DBService): void {
    for (const { model, table, tableName } of this.models) {
