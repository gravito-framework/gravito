...
import type { DatabaseType, DeployOptions, DeployResult, HealthCheckResult, LockOptions, MigrateResult, PaginateOptions, PaginateResult, RelationOptions, SeedFunction, SeedResult, TruncateOptions, UpsertOptions } from './types';
type DrizzleDB = any;
...
export interface DBService {
    ...
    findAll<T>(table: Table, where?: WhereCondition, options?: {
        limit?: number;
        orderBy?: unknown;
    ...
    findAllWith<T>(tableName: string, relations: RelationOptions, options?: {
    ...
    bulkUpdate<T>(table: Table, updates: Array<{
        where: WhereCondition;
        data: Partial<T>;
    }>): Promise<T[]>;
...
export declare class DBServiceImpl implements DBService {
    ...
    private eventBus;
    constructor(db: DrizzleDB, core: PlanetCore, databaseType: DatabaseType, enableQueryLogging: boolean, queryLogLevel: 'debug' | 'info' | 'warn' | 'error', enableHealthCheck: boolean, healthCheckQuery: string);
    get raw(): DrizzleDB;
    /**
