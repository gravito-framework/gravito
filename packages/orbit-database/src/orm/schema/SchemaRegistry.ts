...
export class SchemaRegistry {
    ...
    async get(table: string): Promise<TableSchema> {
        ...
        throw new Error(
            `Table "${table}" not found in schema lock. ` +
            `Run 'bun db:schema:lock' to regenerate.`
        )
    }
    ...
    loadFromLock(path?: string): void {
        ...
        if (!existsSync(lockPath)) {
            throw new Error(
                `Schema lock file not found: ${lockPath}. ` +
                `Run 'bun db:schema:lock' to generate.`
            )
        }
    ...
    /**
     * Refresh schema for a table (re-sniff from database)
     */
