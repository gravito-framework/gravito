...
import type { TableSchema, ColumnType } from '../schema/types'
import { DirtyTracker } from './DirtyTracker'
import { ColumnNotFoundError, TypeMismatchError, NullableConstraintError, ModelNotFoundError } from './errors'
...
export abstract class Model {
    ...
    protected async _performInsert(): Promise<this> {
        ...
        if (Array.isArray(result) && result.length > 0) {
            ...
            if (typeof pk === 'object' && pk !== null) {
                this._attributes[constructor.primaryKey] = (pk as Record<string, unknown>)[constructor.primaryKey]
            } else {
    ...
    static query<T extends Model>(
        ...
        const originalGet = builder.get.bind(builder)
            ; (builder as unknown as { get: () => Promise<T[]> }).get = async function (): Promise<T[]> {
                const rows = await originalGet()
                return rows.map(row => modelClass.hydrate<T>(row)) as unknown as T[]
            }

        // Wrap first() to hydrate result
        const originalFirst = builder.first.bind(builder)
        builder.first = async function (): Promise<T | null> {
            const row = await originalFirst()
            return row ? modelClass.hydrate<T>(row) : null
     ...
     */
    static async count(
        this: ModelConstructor<Model> & typeof Model
    ): Promise<number> {
        const connection = DB.connection(this.connection)
        const result = await connection
            .table(this.table)
