...
export function WorkersPage() {
    ...
    if (isPending) return (
        <div className="flex flex-col items-center justify-center p-20 space-y-6">
            ...
            <p className="text-muted-foreground font-bold uppercase tracking-[0.3em] text-xs">Loading workers...</p>
        </div>
    )
    ...
    if (error) return (
        <div className="text-center p-20">
            <div className="bg-red-500/10 text-red-500 p-10 rounded-3xl border border-red-500/20 max-w-md mx-auto shadow-2xl">
                ...
                <h3 className="text-2xl font-black mb-2 uppercase tracking-tight">Failed to Load Workers</h3>
        ...
        <div className="space-y-8">
            ...
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2 text-sm font-bold opacity-60 uppercase tracking-widest">Monitor and manage cluster processing nodes.</p>
                </div>
                ...
                <div className="card-premium p-6 relative overflow-hidden group">
                    ...
                    <div className="relative">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-widest">Online Nodes</p>
                        </div>
                ...
                <div className="card-premium p-6 relative overflow-hidden group">
                    ...
                    <div className="relative">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-widest">Offline Nodes</p>
                        </div>
                            ...
                            <div className="flex items-center gap-4">
                                <div className="relative">
                                    <div className={cn(
                                        ...
                                        worker.status === 'online' ? "bg-green-500/10 text-green-500" : "bg-muted text-muted-foreground"
                                    )}>
                                        <Cpu size={24} />
                                    </div>
                                    <div className={cn(
                                        "absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-card",
                                        worker.status === 'online' ? "bg-green-500 animate-pulse" : "bg-muted-foreground"
                                    )} />
                                </div>
                                <div>
                                    <h3 className="font-black tracking-tight text-lg group-hover:text-primary transition-colors">{worker.id}</h3>
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">PID: {worker.pid}</p>
                                </div>
                            </div>
                            <span className={cn(
                                "px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border",
                                worker.status === 'online' ? "bg-green-500/10 text-green-500 border-green-500/20" : "bg-muted/40 text-muted-foreground border-transparent"
                            )}>
                                {worker.status}
                            </span>
                        </div>
