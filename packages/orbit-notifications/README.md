...
- **完全模組化**：按需安裝通道，核心包極小
- **隊列化支援**：整合 `@gravito/orbit-queue`，支援異步發送
- **AI 友善**：完整的型別推導、清晰的 JSDoc、直觀的 API

## 安裝

```bash
bun add @gravito/orbit-notifications
```
...
```typescript
import { Notification } from '@gravito/orbit-notifications'
import type { MailMessage, DatabaseNotification, Notifiable } from '@gravito/orbit-notifications'
...
### 2. 配置 OrbitNotifications

```typescript
import { PlanetCore } from 'gravito-core'
import { OrbitNotifications } from '@gravito/orbit-notifications'
...
```typescript
import { Notification, ShouldQueue } from '@gravito/orbit-notifications'
...
需要安裝 `@gravito/orbit-broadcasting`：
